...
<section>
	...
	<article>
		<form action="includes/process.php?action=update" method="post" onsubmit="return updateLocation(this);">
			<input type="url" name="u" placeholder="http://" value="http://" formnovalidate="formnovalidate">
			<input type="submit" value="GO &rarr;">
			<div style="display:none"><?php foreach ($toShow as $option) echo '<input type="checkbox" name="'.$option['name'].'" id="'.$option['name'].'"'.$option['checked'].'>';?></div>
		</form>
	</article>
