	...
	if ( ! strpos($url, '://') ) {
		...
		if ( isset($GLOBALS['unique_salt']) ) {
			...
			if ( $verifyUnique && substr($url, 0, strlen($GLOBALS['unique_salt'])) != $GLOBALS['unique_salt'] ) {
				if (strpos($_SERVER['HTTP_REFERER'], 'http://proxy.org/')!==false) {
					$url = htmlspecialchars_decode('http'.base64_decode($url));
...
function injectionJS() {
	...
	$jsFile = PHPROXY_URL . '/includes/main.js';
